...
def load_excel_sheets(file):
    if file is not None:
        if file.name.endswith('.csv'):
            return {"Data CSV": pd.read_csv(file)}
        else:
            # Membaca SEMUA sheet yang ada di file Excel secara penuh
            xls = pd.ExcelFile(file)
            sheets_dict = {sheet: pd.read_excel(file, sheet_name=sheet) for sheet in xls.sheet_names}
            return sheets_dict
    else:
        ...
        return {"Data Transaksi": dummy_df}

sheets_data = load_excel_sheets(uploaded_file)

# Jika file Excel punya banyak sheet, tampilkan pilihan sheet di sidebar
with st.sidebar:
    st.markdown("---")
    selected_sheet_name = st.selectbox("Pilih Sheet Excel:", options=list(sheets_data.keys()))
    df = sheets_data[selected_sheet_name].dropna(how='all') # Hapus baris kosong

# Standardisasi kolom jika berada di sheet transaksi
...
if 'TOTAL HARGA BELI' in df.columns and 'TOTAL HARGA JUAL' in df.columns:
    ...
    df['Profit'] = df.get('TOTAL HARGA JUAL', 0) * 0.19

# --- SIDEBAR FILTERS (Hanya aktif jika kolom tersedia di sheet tersebut) ---
with st.sidebar:
    st.markdown("---")
    if 'Tahun' in df.columns:
        selected_tahun = st.multiselect("Pilih Tahun:", sorted(df['Tahun'].dropna().unique()), default=sorted(df['Tahun'].dropna().unique()))
    else:
        selected_tahun = []

    pay_col = 'METODE PEMBAYARAN' if 'METODE PEMBAYARAN' in df.columns else ('Pembayaran' if 'Pembayaran' in df.columns else None)
    if pay_col:
        all_pay = sorted(df[pay_col].dropna().unique())
...
# Filter DataFrame
df_f = df.copy()
if 'Tahun' in df_f.columns and selected_tahun:
    df_f = df_f[df_f['Tahun'].isin(selected_tahun)]
if pay_col and selected_pay:
    df_f = df_f[df_f[pay_col].isin(selected_pay)]
if 'Bulan' in df_f.columns and selected_bulan:
    df_f = df_f[df_f['Bulan'].isin(selected_bulan)]

# --- MAIN LAYOUT 3 KOLOM ---
col_left_panel, col_main, col_right_panel = st.columns([1.2, 5, 1.2])

prod_col = 'NAMA PRODUK' if 'NAMA PRODUK' in df.columns else ('Produk' if 'Produk' in df.columns else None)
cat_col = 'KATEGORI' if 'KATEGORI' in df.columns else ('Kategori' if 'Kategori' in df.columns else None)
jenis_col = 'JENIS PENJUALAN' if 'JENIS PENJUALAN' in df.columns else None
rev_col = 'TOTAL HARGA JUAL' if 'TOTAL HARGA JUAL' in df.columns else ('Total_Penjualan' if 'Total_Penjualan' in df.columns else None)
qty_col = 'QTY' if 'QTY' in df.columns else ('Qty' if 'Qty' in df.columns else None)

with col_left_panel:
    if prod_col and qty_col and not df_f.empty:
    ...
    if cat_col and rev_col and not df_f.empty:
...
with col_main:
    ...
    tot_penjualan = df_f[rev_col].sum() if rev_col and not df_f.empty else 0
    tot_profit = df_f['Profit'].sum() if not df_f.empty else 0
    tot_qty = df_f[qty_col].sum() if qty_col and not df_f.empty else len(df_f)
    ...
    if 'Bulan' in df_f.columns and rev_col and not df_f.empty:
        ...
        c_graf1.info("Grafik bulanan memerlukan kolom TANGGAL.")

    if jenis_col and rev_col and not df_f.empty:
        ...
        c_graf2.info("Data jenis penjualan tidak tersedia di sheet ini.")

    c_r2_1, c_r2_2, c_r2_3 = st.columns([1.5, 1.8, 1.7])
    
    if 'Tahun' in df_f.columns and rev_col and not df_f.empty:
    ...
    if pay_col and rev_col and not df_f.empty:
...
with st.expander(f"📄 Lihat Data Lengkap dari Sheet: {selected_sheet_name}"):
